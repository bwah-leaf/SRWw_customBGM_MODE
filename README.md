...
모든 입력 음원은 다음 내부 형식으로 정규화됩니다. 채널 방식은 곡마다 개별 지정할 수 있습니다.

```text
Sample rate : 22,050 Hz
Channels    : Mono 또는 Stereo (트랙별 선택)
...
검증된 원본 ROM의 크기, NDS 최대 ROM 용량 512 MiB, 스트림 정렬 오버헤드와 `22.05 kHz / Mono / PCM16` 규격을 보수적으로 계산한 **모노 환산** 전체 한도는 다음과 같습니다.

```text
02:58:09
```

프로그램은 실제 곡 길이 합계와 채널 수를 반영한 용량 환산 시간을 함께 표시합니다. 스테레오 1초는 모노 2초와 같은 용량으로 계산됩니다. 환산 합계가 최대 시간을 초과하면 `BGM.PAK 만들기` 버튼이 자동으로 비활성화됩니다. 실제 변환 결과가 용량을 초과하는 경우에도 백엔드 검증 단계에서 생성을 중단합니다.
...
5.`변환`에서 모노 또는 스테레오를 고른 뒤 `선택 ID부터 음원 지정`을 눌러 음원을 배정합니다. 여러 파일은 파일명 속 숫자를 인식하는 자연 정렬(`2` 다음 `10`)로 배치되며, 적용 전에 `BGM ID ← 파일명` 목록이 표시됩니다.
6. 이미 지정한 곡의 방식을 바꾸려면 해당 행을 선택하고 `선택 곡에 적용`을 누릅니다. 여러 행을 동시에 선택할 수 있습니다.
7. 필요한 ID가 채워질 때까지 4~6번을 반복합니다.
8. 용량 환산 시간이 한도 이내인지 확인하고 `BGM.PAK 만들기`를 누릅니다.
